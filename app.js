...
async function loadAI(){
  ...
  setStatus("Preparando IA local Qwen2.5 0.5B…");
  try{
    // Prefer WebGPU when available, but fall back automatically to WebAssembly/CPU.
    const device = ("gpu" in navigator) ? "webgpu" : "wasm";
    $("deviceBadge").textContent = device === "webgpu" ? "WEBGPU" : "CPU";
    $("gpuStatus").textContent = device === "webgpu" ? "Disponible" : "No disponible — usando CPU";
    setProgress(5);
    ...
    setProgress(100);
    setStatus(device === "webgpu"
      ? "IA lista con WebGPU."
      : "IA lista en modo CPU. Será más lenta, pero no necesita GPU.");
    $("loadBtn").textContent="✓ IA cargada";
  }catch(e){
    console.error(e);
    setStatus("No se pudo cargar la IA local: "+(e.message||e),true);
...
$("gpuStatus").textContent=("gpu" in navigator)?"Disponible — se intentará acelerar":"No disponible — modo CPU";
