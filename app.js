...
async function renderVideo(){
  ...
  const requestedQuality=Number($("videoQuality")?.value||360);
  const requestedFps=Number($("videoFps")?.value||12);
  const quality=Math.min(Math.max(requestedQuality,360),720);
  const fps=Math.min(Math.max(requestedFps,10),15);
...
$("batchStartBtn")?.addEventListener("click",startAutomaticBatch);
$("batchStopBtn")?.addEventListener("click",stopAutomaticBatch);
...
async function startAutopilot(){
 ...
 const start=$("batchStartBtn"), stop=$("batchStopBtn"); if(start)start.disabled=true;if(stop)stop.disabled=false;
...
async function startAutomaticBatch(){
  ...
  batchRunning=true;batchStop=false;
  const start=$("batchStartBtn"), stop=$("batchStopBtn");
...
async function v15UseAIImagesForVideo(){
  videoImages=[];
  const list=Array.isArray(aiImages)?aiImages.slice(0,8):[];
  for(const x of list){
    try{
      ...
      const img=await loadImageObject(local);
      img.__v15ObjectURL=local;
      videoImages.push(img);
    }catch(e){console.warn("Imagen no disponible para render",e);}
  }
}
...
async function v15MakeVideo(kind,format,duration,topic){
  ...
  if(kind==="short"){
    ...
    await v15UseAIImagesForVideo();
    if(!videoImages.length)throw new Error("Las escenas se generaron, pero Chrome no pudo cargarlas para el video.");
  }
  if(v15Stop)throw new Error("Producción detenida.");
  await generateVoice();
  ...
  const mp4Mode=document.getElementById("v15Mp4Mode")?.value||"auto";
  if(mp4Mode!=="browser" && rendered.ext!=="mp4"){
...
function v15GetDurations(){
  const mode=document.getElementById("v15Mode")?.value||"quick";
  if(mode==="quick") return {short:30,long:60};
  if(mode==="mobile") return {short:45,long:120};
  return {short:Number(document.getElementById("v15ShortDuration")?.value||60),long:Number(document.getElementById("v15LongDuration")?.value||300)};
}
...
async function createV15OneClick(topicOverride){
  ...
  try{
    ...
    const durations=v15GetDurations();
    const shortDur=durations.short;
    const longDur=durations.long;
