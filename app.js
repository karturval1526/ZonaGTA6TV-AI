...
async function renderVideo(){
  ...
  const quality=Number($("videoQuality")?.value||720),fps=Number($("videoFps")?.value||30);
  const sampleRate=voiceAudio.sample_rate||24000;
  const total=voiceAudio.data.length/sampleRate;
  const canvas=document.createElement("canvas");const isLong=String($("format")?.value||"short")==="long";canvas.width=isLong?Math.round(quality*16/9):quality;canvas.height=isLong?quality:Math.round(quality*16/9);
  const ctx=canvas.getContext("2d",{alpha:false});
  const videoStream=canvas.captureStream(fps);

  // Route the generated WAV through an AudioContext so the MediaRecorder captures narration with the video.
  ...
  if(!mime){setVideoStatus("Tu Chrome no ofrece un formato de video compatible.",true);ac.close();return}
  const chunks=[];
  videoRecorder=new MediaRecorder(combined,{mimeType:mime,videoBitsPerSecond:quality>=1080?8000000:5000000,audioBitsPerSecond:128000});
  videoRecording=true;
  $("renderVideoBtn").disabled=true;$("stopVideoBtn").disabled=false;$("downloadVideoBtn").disabled=true;
  setVideoProgress(0);setVideoStatus("Renderizando video con narración + subtítulos…");
  ...
  const frame=now=>{
    ...
    const elapsed=Math.min(total,(now-start)/1000),pct=elapsed/total*100;
    ...
    setVideoProgress(pct);setVideoStatus(`Renderizando ${Math.floor(pct)}% — ${formatTime(elapsed)} / ${formatTime(total)}`);
    if(elapsed<total&&videoRecording)videoAnimation=requestAnimationFrame(frame);
    else{videoRecording=false;try{videoRecorder.stop()}catch{}}
  };
  ...
  setVideoProgress(100);setVideoStatus(`✓ Video ${ext.toUpperCase()} creado con voz y subtítulos sincronizados.`);
}
function stopVideo(){
  if(!videoRecording)return;
